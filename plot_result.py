...
def plot_weak(done, show=False):
    ...
    for i in range(2):
        ...
        plt.plot(groups[i].nodes, groups[i].Mv, '-', label="Mv " + suffix, linewidth=1.5, marker='o', markerfacecolor='none', markersize=15)
        ymax = max(ymax, max(groups[i].Av), max(groups[i].Mv))
    plt.xscale('log')
    plt.xlabel("number of nodes", fontsize=label_size)
    ...
    for i in range(2):
        ...
        plt.plot(groups[i].nodes, groups[i].Mv_eff, '-', label="Mv " + suffix, linewidth=1.5, marker='o', markerfacecolor='none', markersize=15)
    plt.xscale('log')
    plt.xlabel("number of nodes", fontsize=label_size)
    plt.ylabel("efficiency", fontsize=label_size)
    ax.xaxis.set_minor_formatter(x_fmt)
    ax.xaxis.set_major_formatter(x_fmt)
    plt.grid(True, which='both')
...
def plot_fix(done, show=False):
    ...
    plt.xticks(size, size, fontsize = tick_size)
    plt.xlabel('problem size', fontsize = label_size)
    plt.yticks(fontsize = tick_size)
    plt.ylabel('degree', fontsize = label_size)
    ...
    plt.xlabel('problem size', fontsize = label_size)
    plt.yticks(fontsize = tick_size)
    plt.ylabel('iterations', fontsize = label_size)
    plt.grid(True, which='both', c='xkcd:light grey')
    ...
    plt.xlabel('problem size', fontsize = label_size)
    plt.yticks(fontsize = tick_size)
    plt.ylabel('total time (s)', fontsize = label_size)
    plt.grid(True, which='both', c='xkcd:light grey')
    ax.xaxis.set_major_formatter(f_fmt)
    ax.xaxis.set_minor_formatter(NullFormatter())
    plt.tight_layout()
    ...
    plt.xticks([1e6, 4e6, 16e6, 64e6], [1e6, 4e6, 16e6, 64e6], fontsize = tick_size)
    plt.xlabel('problem size', fontsize = label_size)
    plt.yticks(fontsize = tick_size)
    plt.ylabel('unified time (s)', fontsize = label_size)
...
def plot_strong(done, show=False):
    ...
    bx.set_ylabel("efficiency", fontsize = label_size)
    ...
    plt.xticks(C3_nn, C3_nn, fontsize = tick_size)
    plt.yticks(fontsize = tick_size)
    plt.tight_layout()
    ...
    bx.xaxis.set_major_formatter(x_fmt)
    bx.legend(loc='upper right', fontsize = 15)
    plt.xticks(M_nn[0], M_nn[0], fontsize = tick_size)
    plt.yticks(fontsize = tick_size)
    plt.tight_layout()
